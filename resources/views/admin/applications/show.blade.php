...
<div class="flex items-center justify-between mb-8">
    <h2 class="text-2xl font-bold text-text">{{ __('messages.application_details') }}</h2>
        ...
        <div class="bg-surface rounded-xl border border-border">
            ...
            <div class="p-6">
                <form action="{{ route('admin.applications.status', $application) }}" method="POST" class="mb-6">
                    @csrf
                    <label class="block text-sm font-medium text-text mb-1.5">{{ __('messages.update_status') }}</label>
                    ...
                    <button type="submit" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-primary text-white rounded-lg hover:bg-primary-hover transition-colors text-sm font-medium">{{ __('messages.update_status') }}</button>
                </form>
                ...
                @if($application->status !== 'interview_scheduled' && $application->status !== 'approved' && $application->status !== 'rejected')
                    <div class="border-t border-border pt-6 mt-2">
                        <form action="{{ route('admin.applications.interview', $application) }}" method="POST">
                            @csrf
                            <label class="block text-sm font-medium text-text mb-1.5">{{ __('messages.schedule_interview') }}</label>
                            ...
                            <button type="submit" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-info text-white rounded-lg hover:bg-blue-600 transition-colors text-sm font-medium">{{ __('messages.schedule_interview') }}</button>
                        </form>
                    </div>
                @endif
            </div>
