...
<div class="flex items-center justify-between mb-8">
    <h2 class="text-2xl font-bold text-text">Edit Program</h2>
    ...
    <div class="p-6">
        <form action="{{ route('admin.programs.update', $program) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                ...
                <div class="lg:col-span-1"><label class="block text-sm font-medium text-text mb-1.5">Slug</label><input type="text" name="slug" class="w-full rounded-lg border border-border px-3 py-2 text-sm text-text bg-surface focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" value="{{ $program->slug }}"></div>
                
                <div class="lg:col-span-1"><label class="block text-sm font-medium text-text mb-1.5">Service</label><select name="service_id" class="w-full rounded-lg border border-border px-3 py-2 text-sm text-text bg-surface focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"><option value="">None</option>@foreach($services as $s)<option value="{{ $s->id }}" {{ $program->service_id==$s->id?'selected':'' }}>{{ $s->name_en }}</option>@endforeach</select></div>
                ...
                <div class="lg:col-span-1"><label class="block text-sm font-medium text-text mb-1.5">Level</label><select name="level" class="w-full rounded-lg border border-border px-3 py-2 text-sm text-text bg-surface focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"><option value="">None</option><option value="beginner" {{ $program->level=='beginner'?'selected':'' }}>Beginner</option><option value="intermediate" {{ $program->level=='intermediate'?'selected':'' }}>Intermediate</option><option value="advanced" {{ $program->level=='advanced'?'selected':'' }}>Advanced</option></select></div>
                ...
                <div class="lg:col-span-1"><label class="block text-sm font-medium text-text mb-1.5">Currency</label><select name="currency" class="w-full rounded-lg border border-border px-3 py-2 text-sm text-text bg-surface focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"><option value="SAR" {{ $program->currency=='SAR'?'selected':'' }}>SAR</option><option value="USD" {{ $program->currency=='USD'?'selected':'' }}>USD</option></select></div>
                
                <div class="lg:col-span-1"><label class="block text-sm font-medium text-text mb-1.5">Start Date</label><input type="date" name="start_date" class="w-full rounded-lg border border-border px-3 py-2 text-sm text-text bg-surface focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" value="{{ $program->start_date?->format('Y-m-d') }}"></div>
                <div class="lg:col-span-1"><label class="block text-sm font-medium text-text mb-1.5">End Date</label><input type="date" name="end_date" class="w-full rounded-lg border border-border px-3 py-2 text-sm text-text bg-surface focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" value="{{ $program->end_date?->format('Y-m-d') }}"></div>
                <div class="lg:col-span-1"><label class="block text-sm font-medium text-text mb-1.5">Facilitator</label><select name="facilitator_provider_id" class="w-full rounded-lg border border-border px-3 py-2 text-sm text-text bg-surface focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"><option value="">None</option>@foreach($providers as $p)<option value="{{ $p->id }}" {{ $program->facilitator_provider_id==$p->id?'selected':'' }}>{{ $p->user?->full_name }}</option>@endforeach</select></div>
                ...
                <div class="flex items-center gap-2"><input type="checkbox" name="is_featured" value="1" {{ $program->is_featured?'checked':'' }} class="w-4 h-4 text-primary border-border rounded focus:ring-primary"><label class="text-sm font-medium text-text">Featured</label></div>
                <div class="flex items-center gap-2"><input type="checkbox" name="is_active" value="1" {{ $program->is_active?'checked':'' }} class="w-4 h-4 text-primary border-border rounded focus:ring-primary"><label class="text-sm font-medium text-text">Active</label></div>
            </div>
            <button type="submit" class="px-4 py-2.5 bg-primary text-white rounded-lg hover:bg-primary-hover transition-colors text-sm font-medium">Update Program</button>
        </form>
    </div>
</div>
@endsection
